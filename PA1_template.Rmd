...
The data is loaded using the ```{r}read.csv()```.
...
Change the date into dateformat using ```{r}lubridate```:

```r
activity$date <- ymd(activity$date)
```

Check the data with ```{r}names()```,```{r}str()``` and ```{r}head()```:
...
1.Calculate the total number of steps per day using ```{r}dplyr``` and group by ```{r}date```:
...
2.Use ```{r}ggplot2``` for making the histogram:
...
1.Calculate the average number of steps taken in each 5-minute interval per day using ```{r}dplyr``` and group by ```{r}interval```:
...
Use ```{r}ggplot2``` for making the time series of the 5-minute interval and average steps taken:
...
2. Let’s take the approach to fill in a missing ```{r}NA``` with the average number of steps in the same 5-min interval.
...
4.Calculate the number of steps taken in each 5-minute interval per day using ```{r}dplyr``` and group by ```{r}interval```.
  Use ```{r}ggplot2``` for making the histogram:
...
1.Use ```{r}dplyr``` and ```{r}mutate``` to create a new column, weektype, and apply whether the day is weekend or weekday:
...
2.Calculate the average steps in the 5-minute interval and use ```{r}ggplot2``` for making the time series of the 5-minute interval for weekday and weekend, and compare the average steps:
